...
if (source.droppableId === 'available-letters' && destination.droppableId === 'arrangement-zone') {
      ...
      if (!letter || isLetterUsed(letter, letterIndex)) return;
...
<div className="mb-4">
            ...
            <p className="text-sm text-gray-400">Drag letters to arrange your word or click to add • {letters.length} letters total</p>
        </div>
        {/* Word Arrangement Zone */}
        <div
            className="mb-6 p-4 bg-surface-700 rounded-lg border-2 border-dashed border-surface-500">
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-heading text-white">Word Arrangement</h3>
                {arrangedLetters.length > 0 && <button
                    onClick={clearArrangement}
                    className="text-sm text-gray-400 hover:text-white flex items-center gap-1">
                    <ApperIcon name="X" size={14} />Clear
                                  </button>}
            </div>
            <Droppable droppableId="arrangement-zone" direction="horizontal">
                {(provided, snapshot) => <div
                    ...
                    className={`flex gap-2 min-h-[80px] items-center justify-center p-3 rounded-lg transition-colors ${snapshot.isDraggingOver ? "bg-accent/10 border-accent border-2" : "bg-surface-600"}`}>
                    {arrangedLetters.length > 0 ? arrangedLetters.map(
        ...
        <AnimatePresence mode="wait">
            <motion.div
                ...
                {letters.length > 0 ? <Droppable droppableId="available-letters" direction="horizontal">
                    {(provided, snapshot) => <div
                        ref={provided.innerRef}
                        {...provided.droppableProps}
                        className={`flex flex-wrap gap-3 justify-center min-h-[80px] items-center p-3 rounded-lg transition-colors ${snapshot.isDraggingOver ? "bg-surface-700" : ""}`}>
                        {letters.map((letter, index) => {
                            const isUsed = isLetterUsed(letter, index);
                            const uniqueId = `available-${letter}-${index}`;
